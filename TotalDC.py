...
monthlyUnits = [0]*147 #Amount of Month
monthlySales = [0]*147
comicsIn = [0]*147
...
while (year < 2020 or month!=4):
    ...
    monthList.append(monthC+"/"+str(year))
    ...
    for index, row in issueData[dcIssueData].iterrows():
        ...
        sales = row['Sales']
        units = row['Units']
        monthlySales[monthCol]=monthlySales[monthCol]+sales
        monthlyUnits[monthCol]=monthlyUnits[monthCol]+units
        comicsIn[monthCol]=comicsIn[monthCol]+1
...
dcDataFrame = pd.DataFrame(monthlySales,columns=["DC Total"], index = monthList)
dcDataFrame.to_excel("DcIssueTotalSalesData.xlsx")
dcDataFrame = pd.DataFrame(monthlyUnits,columns=["DC Total"], index = monthList)
dcDataFrame.to_excel("DcIssueTotalUnitsData.xlsx")
dcDataFrame = pd.DataFrame(comicsIn,columns=["DC Total"], index = monthList)
dcDataFrame.to_excel("DcIssueTotalComicsInData.xlsx")